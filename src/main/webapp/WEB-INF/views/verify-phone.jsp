...
<title>CommuteSafe – Verify Phone</title>
...
.phone-chip{display:inline-flex;align-items:center;gap:6px;background:#f0fdf4;border:1px solid #86efac;color:#166534;border-radius:20px;padding:4px 14px;font-size:.8rem;font-weight:700;margin-bottom:14px}
</style>
</head>
<body>
<div class="card">
  ...
  <div class="card-body">
    ...
    <div class="card-title">Verify Your Phone Number</div>
    <%
        jakarta.servlet.http.HttpSession sess = request.getSession(false);
        String phone = sess != null ? (String) sess.getAttribute("reg_phone") : null;
        String masked = null;
        if (phone != null && phone.length() > 4) {
            masked = phone.substring(0, phone.length() - 4).replaceAll("\\d", "*") + phone.substring(phone.length() - 4);
        }
    %>
    <% if (masked != null) { %>
    <div class="phone-chip"><i class="bi bi-phone"></i><%= masked %></div>
    <% } %>
    <p class="subtitle">
      We sent a 6-digit SMS code to your mobile number.<br>
      <span style="color:#9ca3af;font-size:.78rem">Enter it below to complete registration. Valid for 10 minutes.</span>
    </p>
    ...
    <form method="post" action="${pageContext.request.contextPath}/users/verify-phone" id="verify-form">
      ...
      <button type="submit" class="btn-main"><i class="bi bi-check-circle-fill"></i> Verify &amp; Create Account</button>
    </form>

    <div class="link-row">
      <a href="${pageContext.request.contextPath}/users/login"><i class="bi bi-arrow-left"></i> Cancel &amp; back to login</a>
    </div>
  </div>
</div>
<script>
