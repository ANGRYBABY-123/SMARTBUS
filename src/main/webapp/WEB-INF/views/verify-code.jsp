...
<div class="card">
  ...
  <div class="card-body">
    ...
    <p class="subtitle">
      ...
      <span style="color:#9ca3af;font-size:.78rem">Valid for 15 minutes. Check your spam folder too.</span>
    </p>

    <% String codeError = (String) request.getAttribute("codeError"); %>
    <% if (codeError != null) { %>
    <div class="alert alert-error"><i class="bi bi-exclamation-triangle-fill" style="flex-shrink:0;margin-top:1px"></i><span><%= codeError %></span></div>
    <% } %>

    <form method="post" action="${pageContext.request.contextPath}/verify-code" id="verify-form">
      <div class="code-inputs">
        <input type="text" maxlength="1" class="code-input" name="d1" id="d1" inputmode="numeric" pattern="[0-9]" required autocomplete="off">
        <input type="text" maxlength="1" class="code-input" name="d2" id="d2" inputmode="numeric" pattern="[0-9]" required autocomplete="off">
        <input type="text" maxlength="1" class="code-input" name="d3" id="d3" inputmode="numeric" pattern="[0-9]" required autocomplete="off">
        <input type="text" maxlength="1" class="code-input" name="d4" id="d4" inputmode="numeric" pattern="[0-9]" required autocomplete="off">
        <input type="text" maxlength="1" class="code-input" name="d5" id="d5" inputmode="numeric" pattern="[0-9]" required autocomplete="off">
      </div>
      <button type="submit" class="btn-main"><i class="bi bi-shield-check"></i> Verify Code</button>
    </form>

    <div class="link-row">
      <a href="${pageContext.request.contextPath}/forgot-password"><i class="bi bi-arrow-clockwise"></i> Didn't receive it? Resend code</a>
      <a href="${pageContext.request.contextPath}/users/login" style="color:#9ca3af;font-weight:400"><i class="bi bi-arrow-left"></i> Back to login</a>
    </div>
  </div>
</div>
<script>
...
inputs.forEach(function(inp, i) {
  ...
  inp.addEventListener('paste', function(e) {
    ...
    Array.from(pasted).slice(0,5).forEach(function(ch, j) { if (inputs[i+j]) inputs[i+j].value = ch; });
